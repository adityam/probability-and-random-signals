...
μ^{(0)}_i=\PR(X_0=i),
$$
and its transition matrix $P$. Indeed, the chain rule and the Markov property give
\begin{align*}
...
11. We now provide a general formula for computing mean hitting times. 
    ...
    In the two-absorbing-state example above, the equation for state $1$ would be $m^A_1=1+m^A_1$, which has no finite solution. Indeed, the actual mean hitting time is $m^A_1=∞$.
...
10. For the Ehrenfest urn model in @exm-Ehrenfest-diffusion-model, the invariant distribution is
   ...
   $$
   Indeed, the probability flows between every pair of adjacent states agree:
   $$
