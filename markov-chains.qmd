...
Let $\ALPHABET X$ be a finite set. A stochastic process $\{X_n\}_{n \ge 0}$, $X_n \in \ALPHABET X$, is called a **Markov chain** if it satisfies the _Markov property_: for any $n \in \integers_{\ge 0}$ and any $x_{0:n+1} \in \ALPHABET X^{n+2}$ such that $\PR(X_{0:n}=x_{0:n})>0$, we have
\begin{equation}\tag{Markov property}\label{eq:Markov}
  \PR(X_{n+1} = x_{n+1} \mid X_{0:n} = x_{0:n})
...
The Markov property also holds when a chain is viewed backward over a finite time horizon; @exr-time-reversal asks you to prove this. However, even if the forward chain is time-homogeneous, its backward transition probabilities may depend on time. If the chain is initialized with an invariant distribution, then the reversed chain is also time-homogeneous.

A common way to construct a Markov chain is through a state recursion
$$
X_{n+1}=f_n(X_n,W_n),
$$
where $W_0,W_1,\ldots$ are independent random variables and are independent of $X_0$. Since $W_n$ is independent of the history up to time $n$,
$$
\PR(X_{n+1}=j\mid X_{0:n}=x_{0:n})
=\PR(f_n(x_n,W_n)=j),
$$
which depends on the past only through the current state $x_n$. If $f_n=f$ for all $n$ and the random variables $W_n$ are identically distributed, then the resulting Markov chain is time-homogeneous. This construction will reappear when we study stochastic state-space models.

Conversely, every finite-state time-homogeneous Markov chain can be constructed in this way. Order the state space as $\ALPHABET X=\{0,1,\ldots,K\}$, let $W_n$ be i.i.d.\ uniform random variables on $[0,1)$, and, when $X_n=i$, choose the unique state $j$ such that
$$
  \sum_{k<j}P_{ik}\le W_n<\sum_{k\le j}P_{ik}.
$$
This is the inverse-transform construction: it realizes any finite transition matrix as a state recursion driven by i.i.d.\ noise.
...
The following urn model was introduced by @Ehrenfest1907 to illustrate the probabilistic behavior underlying Boltzmann's $H$-theorem.

There are $K$ labelled particles, each of which is in either compartment $A$ or compartment $B$. At each time, one particle is chosen uniformly at random and moved to the other compartment. Thus, if there are $X_n=i$ particles in compartment $A$ at time $n$, then $X_{n+1}=i-1$ with probability $i/K$ and $X_{n+1}=i+1$ with probability $(K-i)/K$.
:::
...
:::{#exm-state-augmentation}
#### State augmentation

Sometimes a process $\{X_n\}_{n\geq 0}$ is not Markov but can be made Markov by including additional information in the state. This is called **state augmentation**.

For instance, consider a wireless communication channel whose observed state is $X_n\in\{0,1\}$, where $0$ denotes a bad channel and $1$ denotes a good channel. From the good state, the channel transitions to the bad state with probability $p$ and otherwise remains good. Once the channel enters the bad state, it remains bad for exactly two time steps and then returns to the good state. This is a simple model of burst erasures.

The process $\{X_n\}_{n\geq 0}$ is not Markov:
$$
\PR(X_{n+1}=1\mid X_n=0,X_{n-1}=1)=0,
\qquad
\PR(X_{n+1}=1\mid X_n=0,X_{n-1}=0)=1.
$$
Thus, knowing only that the channel is currently bad is not sufficient to determine its next-state distribution; we must also know whether this is the first or second bad time step.

We can make the process Markov by augmenting the state space to
$$
\ALPHABET X=\{(0,0),(0,1),1\},
$$
where $(0,0)$ denotes the first bad time step and $(0,1)$ denotes the second bad time step. The augmented Markov chain is shown below.

![State augmentation for a burst-erasure channel](figures/svg/markov-examples12.svg)

In general, identifying an appropriate state is an important component of stochastic modeling.
:::
...
The Markov chain is called **time-homogeneous** if the right hand side of \eqref{eq:Markov} does not depend on $n$. In this case, we describe the Markov chain by a **state transition matrix** $P$, where
$$
P_{ij}=\PR(X_{n+1}=j\mid X_n=i).
$$
The matrix $P$ is row stochastic:
$$
P_{ij}\ge0,
\qquad
\sum_{j\in\ALPHABET X}P_{ij}=1
\quad\text{for every }i\in\ALPHABET X.
$$

A time-homogeneous Markov chain is completely specified by its initial distribution $μ^{(0)}$, where
$$
μ^{(0)}_i=\PR(X_0=i),
$$
and its transition matrix $P$. The chain rule and the Markov property give
\begin{align*}
&\PR(X_0=x_0,X_1=x_1,\ldots,X_n=x_n)\\
&\quad=μ^{(0)}_{x_0}
P_{x_0x_1}P_{x_1x_2}\cdots P_{x_{n-1}x_n}.
\end{align*}
Thus, $μ^{(0)}$ and $P$ determine all the finite-dimensional distributions of the process. Conversely, any probability distribution $μ^{(0)}$ and row-stochastic matrix $P$ define a time-homogeneous Markov chain.

Such Markov chains can also be visualized using **state transition diagrams**, as illustrated in the examples below.
...
The Ehrenfest urn model presented in @exm-Ehrenfest-diffusion can be modelled as a Markov chain with state space $\{0, 1, \dots, K\}$ and transition probability
$$
  P_{ij} = \begin{cases}
    i/K     & j = i-1 \\
    (K-i)/K & j = i + 1 \\
...
![Ehrenfest urn model](figures/svg/markov-examples2.svg)
...
The above analysis appears to be a bit of black magic. To understand what is going on, note that we are interested in computing $μ^{(0)} P^n$. An efficient way to compute $P^n$ is through an eigendecomposition. Since $P$ is a row-stochastic matrix, we have $P \mathbf{1} = \mathbf{1}$, where $\mathbf{1}$ is the all-ones vector. Thus, $λ_1 = 1$ is always an eigenvalue of _any_ transition matrix, with corresponding eigenvector $\mathbf{1}$.
...
Assume that $a+b>0$. Then, from the eigendecomposition, we have
$$
P^n=VΛ^nV^{-1},
$$
where
...
V=\MATRIX{1&a\\1&-b},
\qquad
Λ=\MATRIX{1&0\\0&1-a-b},
\qquad
V^{-1}=\frac1{a+b}\MATRIX{b&a\\1&-1}.
$$
Let $μ^{(0)}=\MATRIX{μ^{(0)}_0&1-μ^{(0)}_0}$ and define
$$
d=μ^{(0)}_0-\frac{b}{a+b}.
$$
Then,
...
&=μ^{(0)}VΛ^nV^{-1}\\
&=\MATRIX{1&(a+b)d}
  \MATRIX{1&0\\0&(1-a-b)^n}
  \frac1{a+b}\MATRIX{b&a\\1&-1}\\
&=\MATRIX{\dfrac1{a+b}&d(1-a-b)^n}
  \MATRIX{b&a\\1&-1}\\
&=\MATRIX{
\dfrac{b}{a+b}+d(1-a-b)^n
&
\dfrac{a}{a+b}-d(1-a-b)^n
}.
\end{align*}
This gives both components of the occupancy distribution and agrees with the recursion derived above.
:::
...
4.  It can be shown that a state $i$ is recurrent if and only if
    ...
    \sum_{m=1}^{∞}P^{(m)}_{ii}=∞.
    $$
    This follows from the strong Markov property, which is stated in @thm-strong-Markov.
...
11.  A Markov chain is **aperiodic** if all states are aperiodic. A simple sufficient (but not necessary) condition for an irreducible Markov chain to be aperiodic is that there exists a state $i$ such that $P_{ii}>0$. For a finite, irreducible, and aperiodic Markov chain, there exists a positive integer $M$ such that
    $$
    P^{(m)}_{ij}>0,
    \qquad
    \forall m\ge M,\quad i,j\in\ALPHABET X.
    $$
...
2. Let $A$ be a subset of $\ALPHABET X$. The **hitting time** $H^A$ of $A$ is the extended-valued random variable $H^A \colon Ω \to \{0,1,\ldots\}\cup\{∞\}$ given by
   $$
     H^A(ω)=\inf\{n\ge0:X_n(ω)\in A\}.
   $$
   The standard convention is that the infimum of the empty set is $∞$. Thus, $H^A(ω)=∞$ if the sample path never enters $A$. For a state $j \in \ALPHABET X$, we use the shorthand $H^j$ to denote $H^{\{j\}}$.

   To formalize the special role of hitting times, let
   $$
   \mathcal F_n=\sigma(X_0,\ldots,X_n)
   $$
   denote the information available up to time $n$. A random time $τ$ is called a **stopping time** if
   $$
   \{τ\le n\}\in\mathcal F_n,
   \qquad n\ge0.
   $$
   In other words, by time $n$ we can determine whether $τ$ has occurred. A hitting time is a stopping time because
   $$
   \{H^A\le n\}=\bigcup_{k=0}^n\{X_k\in A\}\in\mathcal F_n.
   $$

   :::{#thm-strong-Markov}
   #### Strong Markov property
   Let $τ$ be a stopping time. On the event $\{τ<∞\}$, for every $k\ge0$ and $j\in\ALPHABET X$,
   $$
   \PR(X_{τ+k}=j\mid\mathcal F_τ)
   =P^{(k)}_{X_τj},
   $$
   where $\mathcal F_τ$ denotes the information available up to time $τ$. Thus, conditional on the state $X_τ$, the process after $τ$ evolves like a fresh copy of the Markov chain started from $X_τ$.
   :::
...
4. The mean time taken for the Markov chain to reach $A$ is given by
   $$
     ...
     =\sum_{n=0}^{∞}\PR_i(H^A>n),
   $$
   where the expectation and the sum are allowed to be infinite. This is called the **mean hitting time**.
...
6. 
   ...
   Let $p$ denote the probability of heads and $q = 1-p$ denote the probability of tails. We initialize the Markov chain in state $0$, immediately before the first toss. After the first toss and until the game ends, the state records the most recent toss; once two identical tosses occur consecutively, the chain enters the corresponding absorbing state. Thus, state $0$ is visited only at time $0$. For example, after $HT$ the state is $-1$, and after $TH$ the state is $1$; the chain does not return to state $0$.

   The Markov chain has five states:
   - State $-2$: two consecutive tails (Player 2 wins, absorbing state)
   - State $-1$: the last toss was a tail, but two consecutive tails have not occurred
   - State $0$: the pre-toss state, occupied only at time $0$
   - State $1$: the last toss was a head, but two consecutive heads have not occurred
   ...
   \begin{align*}
     h^2_{-2} &= 0, \\
     h^2_{-1} &= p h^2_1 + q h^2_{-2} = p h^2_1, \\
     h^2_0 &= p h^2_1 + q h^2_{-1}, \\
     h^2_1 &= p h^2_2 + q h^2_{-1} = p + q h^2_{-1}, \\
     h^2_2 &= 1.
   \end{align*}
   From $h^2_{-1}=p h^2_1$ and $h^2_1=p+q h^2_{-1}$, we obtain
   $$
   h^2_1=p+pq h^2_1,
   \qquad
   h^2_1=\frac{p}{1-pq},
   \qquad
   h^2_{-1}=\frac{p^2}{1-pq}.
   $$
   Therefore,
   $$
   h^2_0=p h^2_1+q h^2_{-1}
   =\frac{p^2(1+q)}{1-pq}.
   $$
   For a fair coin, $p=q=\tfrac12$, and hence $h^2_0=\tfrac12$.
   :::
...
7. We now provide a general formula for computing hitting probabilities.
   ...
   The hitting probabilities $\{h^A_i\}_{i \in \ALPHABET X}$ satisfy the following system of linear equations:
   $$
     h^A_i = \begin{cases}
       1, & i \in A \\
     ...
     \end{cases}
   $$
   The hitting probabilities are the *minimal* non-negative solution of this system. The solution is unique if $A$ is reached almost surely from every state under consideration.
   :::
   ...
   Finiteness of the state space alone does not imply uniqueness. For example, let
   $$
     P=\MATRIX{1&0\\0&1},
     \qquad A=\{0\}.
   $$
   The equations are $h^A_0=1$ and $h^A_1=h^A_1$, so every vector $(1,c)$ with $0\le c\le1$ is a solution. The actual hitting probabilities are $(1,0)$, the minimal non-negative solution.
...
8. 
   ...
   $$
     h_i
     =\frac{\Bigl(\frac qp\Bigr)^i-\Bigl(\frac qp\Bigr)^K}
            {1-\Bigl(\frac qp\Bigr)^K},
   $$
   and for $p = q = \frac 12$,
   $$
     h_i=1-\frac{i}{K}.
...
10. 
    ...
    \begin{align*}
      ...
      m^2_2 &= 0.
    \end{align*}
    From the second equation, $m^2_1=1+q m^2_0$. Substituting this into the first equation gives
    $$
    m^2_0=1+q m^2_0+p(1+q m^2_0)
    =1+p+q(1+p)m^2_0.
    $$
    Since $1-q(1+p)=p^2$, we obtain
    $$
    m^2_0=\frac{1+p}{p^2}.
    $$
    :::
...
11. We now provide a general formula for computing mean hitting times. 
    ...
    #### Mean hitting times
    The mean hitting times $\{m^A_i\}_{i \in \ALPHABET X}$ satisfy the following system of linear equations:
    $$
      m^A_i = \begin{cases}
        0, & i \in A \\
      ...
      \end{cases}
    $$
    If $\ALPHABET X$ is finite and $A$ is reached almost surely from every state under consideration, then the mean hitting times are finite and form the unique finite solution of this system. Otherwise, some mean hitting times may be infinite, and the finite system need not have a solution.
    :::
    ...
    In the two-absorbing-state example above, the equation for state $1$ would be $m^A_1=1+m^A_1$, which has no finite solution: the actual mean hitting time is $m^A_1 = ∞$.
...
14. **Fundamental matrix:** Suppose that, starting from any state in $\ALPHABET T$, the chain is eventually absorbed in $\ALPHABET C$ with probability $1$. Then $Q^n \to 0$, or equivalently the spectral radius of $Q$ is less than $1$. Therefore, the **fundamental matrix**
    $$
      N = (I - Q)^{-1} = I + Q + Q^2 + Q^3 + \cdots
    $$
    is well-defined. Merely having an absorbing state is not sufficient: that absorbing state must be reached with probability $1$ from every state included in $\ALPHABET T$.
...
15. **Interpretation of $N_{ij}$:** The entry $N_{ij}$ represents the **expected number of visits to transient state $j$** before absorption, starting from transient state $i$. This can be seen from the series expansion.
    Let $H^{\ALPHABET C}$ denote the time of absorption. Then
    \begin{align*}
      N_{ij}
      &= \sum_{n=0}^∞ [Q^n]_{ij}
       = \sum_{n=0}^∞ \PR_i(X_n=j,\ n<H^{\ALPHABET C}) \\
      &= \EXP_i\left[\sum_{n=0}^{H^{\ALPHABET C}-1}\IND{X_n=j}\right].
    \end{align*}
    Thus, the count includes the visit at time $0$; in particular, a chain starting in state $i$ contributes one visit to state $i$ immediately.
...
7. **PageRank.** Suppose that there are $N$ webpages with links between them, and that every page has at least one outgoing link. Let $\widehat P_{ij}$ denote the fraction of the outgoing links from page $i$ that point to page $j$. A user behaves as follows: with probability $α$, the user chooses one of the outgoing links on the current page, and with probability $1-α$, the user chooses one of the $N$ pages uniformly at random. The transition matrix of the webpages visited by the user is
   $$
   P=α\widehat P+(1-α)\frac{1}{N}\ONES\ONES^{\mathsf T},
   \qquad 0\le α<1.
   $$
   Since every entry of $P$ is positive, this Markov chain is irreducible and aperiodic. Therefore, it has a unique limiting distribution $π$. The rank $π_j$ of page $j$ is the long-run fraction of time that the user spends on that page.

8. **Computing invariant distributions.** 
...
9. As an example, let's compute the invariant distribution of the on-off Markov chain from @exm-on-off. The transition matrix is
   $$
     P = \MATRIX{ 1 - a & a \\ b & 1 - b }.
   $$
...
10. For the Ehrenfest urn model in @exm-Ehrenfest-diffusion-model, the invariant distribution is
   $$
     π_i=2^{-K}\binom Ki,
     \qquad i\in\{0,1,\ldots,K\}.
   $$
   The probability flows between every pair of adjacent states agree:
   $$
     π_i\frac{K-i}{K}=π_{i+1}\frac{i+1}{K}.
   $$
   These equalities, for $i=0,1,\ldots,K-1$, imply the balance equations $π=πP$.

11. We can write balance equations directly from the state transition diagram. For each state $j$, the balance equation states that the probability flow into state $j$ equals the probability flow out of state $j$. Consider the three-state Markov chain with transition matrix
   $$
     P = \MATRIX{ 0.3 & 0.7 & 0 \\
                  0 & 0.3 & 0.7 \\
                  0.7 & 0 & 0.3 }.
...
12. The above example is an illustration of a more general principle. If $P$ is doubly stochastic (i.e., both row sum and column sums are $1$), then the invariant distribution is a uniform distribution.

13. **Exercises: Computing Invariant Distributions**
...
3. In fact, we have a stronger result: the **Strong Law of Large Numbers (SLLN) for finite Markov chains.** Let $\{X_n\}_{n \ge 0}$ be an irreducible Markov chain. Then, for any initial distribution,
   $$
   \frac{1}{N} \sum_{n=1}^N \IND\{X_n = j\} \xrightarrow{a.s.} \pi_j, \quad \forall j \in \ALPHABET X.
   $$
   This is known as the **ergodic property**: sample average equals ensemble average. Notice that aperiodicity is not required for this result.

4. An immediate consequence of the ergodic property is that given an irreducible finite Markov chain $\{X_n\}_{n \ge 0}$ and any function $f \colon \ALPHABET X \to \reals$, we have
   $$
   \frac{1}{N} \sum_{n=1}^N f(X_n) \xrightarrow{a.s.} \sum_{j\in \ALPHABET X} \pi_j f(j).
   $$
 
5. If the chain is periodic with period $d > 1$ (i.e., not aperiodic), the limiting distribution $\lim_{n \to \infty} P^n$ does not exist in the usual sense, as the transition probabilities continue to oscillate. The Ehrenfest chain illustrates this distinction: every transition changes the parity of the state, so the chain has period two. It has the invariant distribution computed earlier, but $P^n$ starting from a fixed state does not converge to this distribution. Nevertheless, since the chain is irreducible, the time averages in the previous two bullets converge along the full sequence $N \to \infty$. Thus, aperiodicity is needed for the convergence of $P^n$ to a matrix whose rows are $π$, but not for the Markov-chain law of large numbers.

## Censored Markov chains

1. Suppose a Markov chain is only observed when the state lies in a subset $\ALPHABET O \subset \ALPHABET X$.  Let $\{τ_k\}_{k \ge 1}$ denote the \emph{stopping times} when the Markov chain returns to the set $\ALPHABET O$, i.e., $τ_1$ is the first time when the Markov chain $\{X_n\}_{n \ge 1}$ hits $\ALPHABET O$; $τ_2$ denotes the second time when the Markov chain hits $\ALPHABET O$; and so on.

2. Define the process $\{O_k\}_{k \ge 1}$ by $O_k = X_{τ_k}$. The **strong Markov property** implies that $\{O_k\}_{k \ge 1}$ is a Markov chain. What is the transition probability matrix $\tilde P$ of this chain?

3. As we did for the absorbing Markov chains, we re-order states so that the unobservable states $\ALPHABET O^c$ come first, followed by all observable state $\ALPHABET O$. Then the transition matrix $P$ can be written as
   $$
      P = \MATRIX{
      Q & R \\ 
      S & P_O
...
5. Suppose we start with a state $O_1 = X_1 \in \ALPHABET O$. Then the probability of going to each of the states in $\ALPHABET O$ in one step is given by $P_O$. To take more than one step, the chain must enter a state in $\ALPHABET O^c$ with probability $S$. We will assume that the set $\ALPHABET O^c$ does not have any obsorbing subsets. Then, as we showed in the analysis for absorbing Markov chains, starting from a given state in $\ALPHABET O^c$, the Markov chain enters $\ALPHABET O$ for the first time with probability $NR = (I - Q)^{-1} R$. Thus, we have that
   $$ 
      \tilde P = P_O + S(I - Q)^{-1}R.
   $$

6. If $P$ has only one communicating class, then $\{X_n\}_{n \ge 1}$ has a unique invariant distribution, which we denote by $π$. Then $(π_k)_{k \in \ALPHABET O}$ normalized to have sum $1$ is the unique invariant distribution of $\{O_n\}_{n \ge 1}$.

7. As an example, consider the reflected random walk from @exm-random-walk (point 2) with state space $\{-2, -1, 0, 1, 2\}$ 

   ![Reflected random walk](figures/svg/random-walk2.svg)

   Suppose we only observe the process when $X_n \in \ALPHABET O = \{0, 1, 2\}$, so the unobservable states are $\ALPHABET O^c = \{-2, -1\}$.

   Since the states are already ordered in the right order, we can write the transition matrix as:
   $$
   P = \left[\begin{array}{cc|ccc} 0 & 1 & 0 & 0 & 0 \\
              q & 0 & p & 0 & 0 \\ \hline
              0 & q & 0 & p & 0 \\
              0 & 0 & q & 0 & p \\
   ...
   \begin{align*}
   \tilde P &= P_O + S(I - Q)^{-1} R \\
   &= \MATRIX{ 0 & \tfrac{1}{4} & 0 \\ \tfrac{3}{4} & 0 & \tfrac{1}{4} \\ 0 & 1 & 0 } + \MATRIX{ 0 & \tfrac{3}{4} \\ 0 & 0 \\ 0 & 0 } \MATRIX{ 1 & -1 \\ -\tfrac{3}{4} & 1 }^{-1} \MATRIX{ 0 & 0 & 0 \\ \tfrac{1}{4} & 0 & 0 } \\
   &= \MATRIX{ \tfrac{3}{4} & \tfrac{1}{4} & 0 \\ \tfrac{3}{4} & 0 & \tfrac{1}{4} \\ 0 & 1 & 0 }.
   \end{align*}
   ...
   The invariant distribution of $\tilde P$ is 
   $$\MATRIX{ \tfrac{12}{17} & \tfrac{4}{17} & \tfrac{1}{17} }$$
   while the invariant distribution of $P$ is
   $$\MATRIX{ \tfrac{27}{80} & \tfrac{36}{80} & \tfrac{12}{80} & \tfrac{4}{80} & \tfrac{1}{80} }.$$
...
$$
Thus, a time-reversed Markov chain is also Markov. Explain why the reverse transition probabilities may depend on time even when the forward chain is time-homogeneous.
:::
...
2. [Grinstead and Snell, Introduction to Probability](https://stats.libretexts.org/Bookshelves/Probability_Theory/Introductory_Probability_(Grinstead_and_Snell)/11%3A_Markov_Chains/11.01%3A_Introduction), Chapter 11 has an excellent treatment of finite Markov chains. Has interesting exercises.

3. [Kemeny and Snell, Finite Markov Chains](https://catalog.hathitrust.org/api/volumes/oclc/528648.html), Chapter 2--4.

4. [Kemeny, Snell, and Knapp, Denumerable Markov Chains](https://archive.org/details/springer_10.1007-978-1-4684-9455-6), a slightly advanced version of the previous book. 

5. [Doyle and Snell, Random Walks and Electrical Networks](https://doi.org/10.5948/UPO9781614440222) shows a facinating relationship between random walks and circuits!

6. [Weber's notes on Markov chains](https://www.statslab.cam.ac.uk//~rrw1/markov/M.pdf)
