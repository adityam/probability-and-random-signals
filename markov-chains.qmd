...
4. A Markov chain can have multiple invariant distributions. For example, consider a Markov chain with two absorbing states $0$ and $1$. The transition matrix is
   $$
     P = \MATRIX{ 1 & 0 \\ 0 & 1 }.
   $$
...
5. An irreducible and aperiodic Markov chain has a **unique** invariant distribution. Such Markov chains are also called **ergodic**. As we will see later, such Markov chains have the property that for any initial distribution $μ^{(0)}$, we have 
$$\lim_{n \to ∞} μ^{(n)}_j = π_j, \quad \forall j \in \ALPHABET X.$$
Thus, an ergodic Markov chain converges to its invariant distribution. For this reason, the invariant distribution is also called the **limiting distribution** or the **steady-state** distribution.

6. **Computing invariant distributions.** 
...
7. As an example, let's compute the invariant distribution of the on-off Markov chain from @exm-on-off. The transition matrix is
   $$
     P = \MATRIX{ 1 - a & a \\ b & 1 - b }.
   $$
...
8. We can write balance equations directly from the state transition diagram. For each state $j$, the balance equation states that the probability flow into state $j$ equals the probability flow out of state $j$. Consider the three-state Markov chain with transition matrix
   $$
     P = \MATRIX{ 0.3 & 0.7 & 0 \\
                  0 & 0.3 & 0.7 \\
                  0.7 & 0 & 0.3 }.
...
9. The above example is an illustration of a more general principle. If $P$ is doubly stochastic (i.e., both row sum and column sums are $1$), then the invariant distribution is a uniform distribution. 

  Note that if the chain is not irreducible, then it may have multiple invariant distributions. 
...
10. **Exercises: Computing Invariant Distributions**
    ...
    :::{#exm-invariant-convex}
    Suppose a Markov chain has $N$ absorbing states and no transitions between them. What does the set of invariant distributions look like? Express your answer in terms of convex combinations.
    :::
...
2. As discussed above, limiting distributions exist for ergodic Markov chains (i.e., irreducible and aperiodic Markov chains): for any initial distribution $μ^{(0)}$, we have 
   $$\lim_{n \to ∞} μ^{(n)}_j = π_j, \quad \forall j \in \ALPHABET X.$$
    ...
    The matrix version of this property is:
    $$
    \lim_{n \to \infty} P^n = 
    \MATRIX{
      \pi \\
...
- If the chain is not aperiodic (i.e., has period $d > 1$), the limiting distribution as above may not exist, but the chain may still have an invariant distribution.
- In the special case where $P$ is doubly stochastic (rows and columns sum to 1), the limiting distribution is uniform: $\pi_j = 1/|\ALPHABET X|$. 
