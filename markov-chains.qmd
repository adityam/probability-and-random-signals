...
5. An irreducible and aperiodic Markov chain has a **unique** invariant distribution. Such Markov chains are also called **ergodic**. As we will see later, such Markov chais have the property that for any initial distribution $μ^{(0)}$, we have 
$$\lim_{n \to ∞} μ^{(n)}_j = π_j, \quad \forall j \in \ALPHABET X.$$
Thus, an ergodic Markov chain converges to its invariant distribution. For this reason, the invariant distribution is also called then **limiting distribution** or the **steady-state** distribution.
...
- In the special case where $P$ is doubly stochastic (rows and columns sum to 1), the limiting distribution is uniform: $\pi_j = 1/|S|$. 
