...
5. A remarkable property of Markov chain is that these quantities can be computed by solving a system of linear equations associated with the transition probability matrix $P$. 
...
10. For absorbing Markov chains, we can use matrix methods to efficiently compute hitting probabilities and mean hitting times. The key idea is to rearrange the transition matrix into a **canonical form** where absorbing states are grouped together.
...
11. **Multi-step transitions:** For the canonical form, we can show by induction that
  $$
...
12. **Fundamental Matrix:** The **fundamental matrix** $N$ is defined as
  $$
...
13. **Interpretation of $N_{ij}$:** The entry $N_{ij}$ represents the **expected number of visits to transient state $j$** before absorption, starting from transient state $i$. This can be seen from the series expansion:
  $$
    N_{ij} = \sum_{n=0}^∞ [Q^n]_{ij} = \sum_{n=0}^∞ \PR_i(X_n = j, \text{ chain hasn't been absorbed by time } n).
  $$

14. **Mean hitting times:** Let $\ONES$ denote a column vector of ones with dimension $|\ALPHABET T|$. Then the mean hitting times (expected time until absorption) starting from each transient state are given by
    $$
      m = N \ONES,
    $$
...
15. **Hitting probabilities:** The hitting probabilities (absorption probabilities) are given by
    $$
      h = N R,
    $$
