...
4. A consequence of the above is that if $X = (X_1, \dots, X_n) \sim \mathcal{N}(\mu, \Sigma)$, then any subvector formed by selecting a subset of the indices is itself multivariate Gaussian, with the corresponding mean vector and covariance matrix.
...
3. **Hilbert space of random variables**: The key idea is to construct a **Hilbert space** of square-integrable random variables. For scalar random variables $X$ and $Y$ with $\EXP[X^2] < ∞$ and $\EXP[Y^2] < ∞$, we define the inner product as
    ...
    For $\reals^n$-valued square-integrable random vectors $X$ and $Y$ (i.e., $\EXP[\|X\|^2] < ∞$ and $\EXP[\|Y\|^2] < ∞$), we define the inner product as
    $$
        \IP{X}{Y} = \EXP[X^\TRANS Y].
    $$
    The corresponding norm is $\NORM{X}^2 = \EXP[\|X\|^2]$. As before, this space is complete and forms a Hilbert space.
...
4. **Orthogonal projection theorem**: A fundamental property of Hilbert spaces is the orthogonal projection theorem. 
   ...
   $$ x = y + z $$
   where $y \in \ALPHABET M$ and $z \perp \ALPHABET M$ (i.e., for any $w \in \ALPHABET M$, $\IP{z}{w} = 0$). This unique element $y$ is called the **orthogonal projection** of $x$ onto $\ALPHABET M$ and has the property that it minimizes the distance:
   $$
       \NORM{x - y} \le \NORM{x - w}, \quad \forall w \in \ALPHABET M.
   $$
   :::
...
6. **Connection to conditional expectation**: The key insight is that for Gaussian random vectors, the orthogonal projection $\hat{X}$ equals the conditional expectation $\EXP[X \mid Y]$. 

   For general random variables, conditional expectation $\EXP[X \mid Y]$ is defined as the (measurable) function $h(Y)$ that minimizes
   $$
   \EXP[ \| X - h(Y)\|^2 ]
   $$
   among all measurable functions $h(\cdot)$. We did not cover it earlier and simply take this as a fact. 

   For Gaussian random vectors, a remarkable property is that the optimal $h$ is **affine** (linear plus constant). This means we can restrict our search to functions of the form $h(Y) = K Y + c$ for some matrix $K$ and vector $c$. Since $\hat{X}$ minimizes the mean-squared error over all affine functions (by the orthogonal projection theorem), it must be the conditional expectation:
   $$
   \hat{X} = \EXP[X \mid Y].
   $$

7. **Deriving the formula**: To find the explicit formula for $\EXP[X \mid Y]$, we write $\hat{X} = K_{\circ} Y + c_\circ$ and find $K_\circ$ and $c_\circ$ from properties of orthogonal projection. First note that from the smoothing property of conditional expectation, we have
   $$
  ...
  \quad \forall K \in \reals^{n \times m}.
   \end{equation}
   We claim that \eqref{eq:orthogonal} is equivalent to 
   ...
   For the ease of notation, define $U = (X - μ_X) - K_{\circ}(Y - μ_Y)$ and $V = Y - μ_Y$. Observe that
   $$
    U^\TRANS KV = \sum_{i=1}^n U_i [KV]_i 
    = \sum_{i=1}^n \sum_{j=1}^m K_{ij} U_i V_j.
   $$
   Thus, 
   \begin{equation}\label{eq:intermediate}
      \EXP[ U^\TRANS K V] 
      = \sum_{i=1}^n \sum_{j=1}^m K_{ij} \EXP[ U_i V_j] 
   \end{equation}
   We now show that \eqref{eq:orthogonal} implies \eqref{eq:claim}. Pick a $(i', j')$ and set $K_{ij} = δ_{ii'} δ_{jj'}$. Then, \eqref{eq:orthogonal} and \eqref{eq:intermediate} imply that
   $$
      \EXP[U_{i'} V_{j'}] = 0.
   $$
   Since $(i',j')$ were arbitrary, the above equation is true for all $(i', j')$. Writing this in vector notation gives \eqref{eq:claim}.

   We now show the other direction. If \eqref{eq:claim} is true, then each term in the summand in the RHS of \eqref{eq:intermediate} is zero, and hence \eqref{eq:orthogonal} is true.
   :::

   Now the claim implies that
   $$
...
8. The **conditional covariance** $\COV(X \mid Y)$ is defined as 
   $$
...
9. **Summary**: For jointly Gaussian random vectors $(X, Y)$ with the joint distribution given above, the conditional distribution is Gaussian and is given by:
   $$
   \bbox[5pt,border: 1px solid]{
