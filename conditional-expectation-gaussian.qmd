...
7. The orthogonal projection is the conditional expectation, i.e.,
   ...
   Since $\hat{X} \in \ALPHABET M$, it is an affine function of $Y$. Hence $X - \hat X$ and $Y$ are jointly Gaussian. Moreover, since $Y \in \ALPHABET M$, $Y$ is orthogonal to $X - \hat X$, i.e., 
   $$
   \EXP[ \IP{X - \hat{X}}{Y}] = 0.
   $$

   Since $X - \hat X$ and $Y$ are jointly Gaussian, uncorrelation implies independence. Hence, $X - \hat{X}$ and $Y$ are independent. Therefore, for any (measurable) function $g$, the independence gives:
   $$
   ...
   Now, [from last section](conditional-expectation.qmd#MMSE), we know that the conditional expectation is the unique function $h(Y)$ such that $X - h(Y)$ is orthogonal to all functions of $Y$. Hence, $\EXP[X \mid Y] = \hat{X}$. 
   :::
