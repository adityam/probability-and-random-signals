...
1. Recall that a random vector $X\in\reals^n$ is **Gaussian** if every linear combination of its components is a scalar Gaussian random variable. If $\mu=\EXP[X]$ and $\Sigma=\COV(X)$, we write
   $$
   X\sim\mathcal N(\mu,\Sigma).
   $$

2. Gaussian random vectors are closed under affine transformations. If $X\sim\mathcal N(\mu,\Sigma)$, $A\in\reals^{m\times n}$, and $b\in\reals^m$, then
   $$
   AX+b\sim\mathcal N(A\mu+b,A\Sigma A^\TRANS).
   $$

3. Consequently, every subvector of a Gaussian random vector is Gaussian. Its mean vector and covariance matrix are obtained by selecting the corresponding entries and blocks of the original mean and covariance.

4. Random vectors $X\in\reals^n$ and $Y\in\reals^m$ are **jointly Gaussian** if their stacked vector is Gaussian. We write its mean and covariance in block form as
   $$
   \MATRIX{X\\Y}
   \sim
   \mathcal N\left(
   ...
   \MATRIX{
   \Sigma_{XX} & \Sigma_{XY}\\
   \Sigma_{XY}^\TRANS & \Sigma_{YY}
   }
   \right).
   $$

5. Jointly Gaussian random vectors are independent if and only if they are uncorrelated:
   $$
   X\independent Y
   \quad\Longleftrightarrow\quad
   \Sigma_{XY}=0.
   $$
   In particular, the components of a Gaussian random vector are mutually independent if and only if its covariance matrix is diagonal.
...
7. The orthogonal projection is the conditional expectation, i.e.,
   ...
   From [the MMSE estimation chapter](mmse-estimation.qmd#MMSE), we know that if a function $h(Y)$ is such that $X - h(Y)$ is orthogonal to all functions of $Y$, then $h(Y)$ is the conditional expectation $\EXP[X \mid Y]$. Hence, $\hat X = \EXP[X \mid Y]$.
   :::
