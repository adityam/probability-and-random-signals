...
$$
where $\Omega \in \mathbb{R}$ is a constant frequency, and $A$ and $B$ are independent Gaussian random variables with $A, B \sim \mathcal{N}(0, \sigma^2)$.
...
- $\EXP[A^2] = \VAR(A) = \sigma^2$
- $\EXP[B^2] = \VAR(B) = \sigma^2$
- $\EXP[AB] = \EXP[A]\EXP[B] = 0$

Therefore:
\begin{align*}
R_X(m,n) &= \sigma^2 [\cos(\Omega m) \cos(\Omega n) + \sin(\Omega m) \sin(\Omega n)] \\
...
\begin{align*}
  ...
  &= R_X(n,n) + \EXP[W_n^2] = R_X(n,n) + \sigma^2.
\end{align*}

Thus, $\mu_X(n) = \mu_X(0) = 0$ and $R_X(n,n) = \sigma^2 n$ for all $n \ge 0$. 
...
Combining the above we have $R_X(n,m) = \sigma^2 \min(n,m)$ for all $n, m \ge 0$. Thus, the process is not sationary.
:::
