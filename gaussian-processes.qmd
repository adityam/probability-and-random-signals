...
1. A discrete-time stochastic process $\{X_n\}_{n \ge 0}$ is called a **Gaussian process** if for any finite collection of time indices $\{n_1, n_2, \dots, n_k\}$ with $n_i \ge 0$, the random vector $(X_{n_1}, X_{n_2}, \dots, X_{n_k})$ has a multivariate Gaussian (normal) distribution.

2. Equivalently, a discrete-time stochastic process $\{X_n\}_{n \ge 0}$ is a Gaussian process if every finite linear combination of the random variables $X_n$ is a Gaussian random variable.
...
1. **Symmetry:** $R_X(m,n) = R_X(n,m)$ for all $m, n \ge 0$.
...
2. **Non-negative definiteness:** For any finite collection $\{n_1, \dots, n_k\}$ with $n_i \ge 0$ and any real numbers $a_1, \dots, a_k$, we have
   $$
   ...
   #### Proof
   Consider the random variable $Y = \sum_{i=1}^k a_i (X_{n_i} - μ_X(n_i))$. Then
   \begin{align*}
   ...
   \end{align*}
   Since $\EXP[Y^2] = \VAR(Y) \ge 0$ (variance is always non-negative), we have
   $$
   \sum_{i=1}^k \sum_{j=1}^k a_i a_j R_X(n_i, n_j) \ge 0.
   $$
   :::

3. **Cauchy-Schwarz inequality:** $|R_X(m,n)|^2 \le R_X(m,m) R_X(n,n)$.
...
4. The variance function is $R_X(n,n) = \VAR(X_n) \ge 0$.
...
A **white noise process** $\{W_n\}_{n \ge 0}$ is a Gaussian process where $\{W_n\}_{n \ge 0}$ is an i.i.d. sequence with $W_n \sim \mathcal{N}(0, \sigma^2)$ for some $\sigma > 0$.
...
For $m, n \ge 0$, without loss of generality, assume $m \le n$. Then:
\begin{align*}
...
:::{.callout-tip}
#### Invariant distribution and relationship to Markov chains

Observe that the process $\{X_n\}_{n \ge 0}$ satisfies the Markov property. So, it is also called a **Gauss Markov process**.  From the results of @exm-AR1-process, observe that 
$$
...
Since $W_k$ are independent with $\EXP[W_k] = 0$ and $\VAR(W_k) = \sigma^2$, we have $\EXP[W_{m-i} W_{n-j}] = \sigma^2$ if $m-i = n-j$ (i.e., $n-m = j-i$), and $0$ otherwise.

Let $d = n-m$. Then:
$$
R_X(m,n) = \sigma^2 \sum_{k=0}^{q} b_k b_{k+|d|}
$$
where we define $b_k = 0$ for $k < 0$ or $k > q$.

For the case when $n \ge m$ (so $d \ge 0$), we need $j = i + d$:
$$
R_X(m,n) = \sigma^2 \sum_{i=0}^{q} b_i b_{i+d}
$$

Since the covariance function is symmetric, for general $m, n \ge 0$:
$$
R_X(m,n) = \sigma^2 \sum_{k=0}^{q} b_k b_{k+|n-m|}
$$
:::
:::
-->
