...
1. In many applications in signal processing and stochastic control, we are interested in the output of a LTI system when the input is a stationary Gaussian process. There are two ways to study such systems: either in the time domain using state space models or in the frequency domain using the transfer function. 
...
4. Thus, these two examples show the output of two LTI systems when the input is white noise. Note that the transfer function for @exm-gaussian-random-walk is not stable (as it has a pole on the unit circle) and as a consequence the output is unstable (i.e., does not converge to an invariant distribution). In contrast, the transfer function for @exm-AR1-process is stable (as it has all poles inside the unit circle) and as a consequence the output is stable (i.e., converges to an invariant distribution).
