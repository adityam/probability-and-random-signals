...
7. The cross-covariance between the output and input is given as follows: 
   \begin{align*}
     R_{YX}(n,m) &= \EXP[Y_n X_m^*] = \EXP\left[\sum_{k=-\infty}^∞ h_{n-k} X_k X_m^*\right] \\
     &= \sum_{k=-\infty}^∞ h_{n-k} \EXP[X_k X_m^*] \\
...
8. The auto-covariance of the output is given by
   \begin{align*}
     R_Y(n,m) &= \EXP[Y_n Y_m^*] = \EXP\left[Y_n \left(\sum_{k=-\infty}^∞ h_{m-k} X_k\right)^*\right] \\
     &= \sum_{k=-\infty}^∞ h_{m-k}^* \EXP[Y_n X_k^*] \\
