...
We first consider conditioning on discrete random variables.
...
6. We can show that conditional PMFs and conditional PDFs satisfy all the properties of PMFs and PDFs. Therefore, we can define conditional expectation $\EXP[ g(X) \mid Y = y ]$ in terms of $P_{X|Y}$ or $f_{X|Y}$. We can similarly define conditional variance $\VAR(X \mid Y = y)$ using the conditional probability law:
$$
...
2. Analogously, when $X$ and $Y$ are jointly continuous with joint PDF $f_{X,Y}$, we can define conditional expectation as 
   $$
...
2. For any random variable $X$, we define the $\ALPHABET G$-measurable random variable
   $$
...
3. When $X = \IND_{A}$, we write the conditional expectation as a conditional probability:
   $$
   \EXP[\IND_{A} \mid \ALPHABET G] = \PR(A \mid \ALPHABET G).
   $$
...
5. If $X_1$ and $X_2$ are jointly distributed random variables and $a_1$ and $a_2$ are constants, then
   $$ \EXP[ a_1 X_1 + a_2 X_2 \mid \ALPHABET G]
   = a_1 \EXP[X_1 \mid \ALPHABET G] + a_2 \EXP[X_2 \mid \ALPHABET G]. $$

6. If $Y$ is another random variable which is $\ALPHABET G$ measurable (i.e., $Y$ takes constant values on the atoms of $\ALPHABET G$), then
   $$
...
\EXP[ X ].
$$
Here, we have taken $\ALPHABET H = \{\emptyset, Ω\}$ as the trivial $σ$-algebra. Observe that the above definition is equivalent to the law of total probability!

### Conditioning on a random variable

1. If $Y$ is a discrete random variable, then $\PR(A \mid Y)$ and $\EXP[X \mid Y]$ may be viewed as shorthand notation for $\PR(A \mid σ(Y))$ and $\EXP[X \mid σ(Y)]$. Similar interpretations hold for conditioning on multiple random variables (or, equivalently, conditioning on random vectors).
...
4. The previous property is used to generalize the definition of conditional expectation to continuous random variables. First, we consider conditioning with respect to a $σ$-algebra $\ALPHABET G \subset \ALPHABET F$, which is not necessarily finite (or countable). 
...
[^non-negative]: We start with non-negative random variables to avoid concerns about the existence of expectations due to the $∞ - ∞$ indeterminacy. A similar definition works in general as long as we can rule out this case. 

5. It can be shown that $\EXP[X \mid \ALPHABET G]$ exists and is unique up to sets of measure zero. Formally, one talks about a "version" of conditional expectation.
...
3. The error $X - \EXP[X \mid Y]$ is **orthogonal** (i.e., uncorrelated) to all functions of $Y$, i.e., for any (measurable) function $g(Y)$, we have
...
4. Conversely, if a function $h(Y)$ is such that the error $X - h(Y)$ is orthogonal to all functions of $Y$, i.e., $\EXP[(X - h(Y)) g(Y)] = 0$ for all functions $g(Y)$, then $h(Y) = \EXP[X \mid Y]$ (almost surely), and hence $h(Y)$ is the MMSE estimator.
...
5. The MMSE estimator is unbiased: $\EXP[X - \EXP[X \mid Y]] = \EXP[X] - \EXP[\EXP[X \mid Y]] = 0$ by the smoothing property.
