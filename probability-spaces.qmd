...
Probability theory is used to model, analyze, and design various engineering systems. Broadly speaking, there are three forms of random phenomena that arise in systems: noise, uncertainty, and randomness.
...
- **Uncertainty.** Uncertainty refers to effects that arise due to lack of information. For example, to bid in an electricity market, a wind farm may need to know how much energy it can generate in the next hour, but this depends on the speed of wind which depends on a lot of factors and is difficult to predict precisely. This lack of knowledge can be modelled as uncertainty and the system modeler may have a quantified belief on that uncertainty based on historical data. Similar examples arise in circuits (where quality of a chip may depend on the vagaries in the manufacturing process). 
...
6. Given a **universal set** $Ω$ and a collection $\{B_1, B_2, \dots, B_m\}$ of subsets of $Ω$, we say that $\{B_1, B_2, \dots, B_m\}$ is a **partition** of $Ω$ if $\{B_1, B_2, \dots, B_m\}$ are pairwise disjoint and their union equals the universal set $Ω$.
   ...
   The following are **not** partitions of $Ω$ [Explain why?]
...
7. In most of our discussion, we will work with a pre-specified universal set $Ω$. In this setting we use $A^c$ (read as the complement of $A$) as shorthand for $Ω\setminus A$. 
...
10. **Limits of sets** {#set-limits} It is often useful to approximate a complicated set by a sequence of simpler sets. If $\{A_n\}_{n \ge 1}$ is a **weakly increasing** sequence, i.e., $A_1 \subseteq A_2 \subseteq A_3 \subseteq \cdots$, we define its limit by
    $$
      \lim_{n \to ∞} A_n = \bigcup_{n=1}^∞ A_n.
    $$
    If $\{B_n\}_{n \ge 1}$ is a **weakly decreasing** sequence, i.e., $B_1 \supseteq B_2 \supseteq B_3 \supseteq \cdots$, we define its limit by
    $$
      \lim_{n \to ∞} B_n = \bigcap_{n=1}^∞ B_n.
    $$
    For example, $[0, a - \tfrac 1n) \nearrow [0,a)$ and $[x, x + \tfrac 1n] \searrow \{x\}$.

    When $\{A_n\}_{n \ge 1}$ is neither increasing nor decreasing, we define
    $$
      \liminf_{n \to ∞} A_n = \bigcup_{n=1}^{∞} \bigcap_{i=n}^{∞} A_i
      \quad\text{and}\quad
      \limsup_{n \to ∞} A_n = \bigcap_{n=1}^{∞} \bigcup_{i=n}^{∞} A_i.
    $$
    Equivalently, $\liminf_{n} A_n$ is the limit of the increasing sequence $C_n = \bigcap_{i=n}^{∞} A_i$, and $\limsup_{n} A_n$ is the limit of the decreasing sequence $D_n = \bigcup_{i=n}^{∞} A_i$. When the two agree, the sequence has a limit.

    Another useful viewpoint uses indicator functions:
    $$
      ω \in \limsup_{n \to ∞} A_n
      \iff \text{$ω$ belongs to infinitely many $A_n$},
    $$
    and
    $$
      ω \in \liminf_{n \to ∞} A_n
      \iff \text{$ω$ belongs to all but finitely many $A_n$}.
    $$
    We will elaborate on this viewpoint [later](convergence-of-random-variables.qmd).

11. Given a universal set $Ω$, a collection $\ALPHABET F \subseteq 2^Ω$ of subsets of $Ω$ is called an **algebra** if it satisfies the following properties:
...
12. We will sometimes use the notation "$(Ω,\ALPHABET F)$ is an algebra of sets" or "$\ALPHABET F$ is an algebra on $Ω$". Some examples of algebras are as follows:
...
13. A finite partition generates an algebra by taking unions of its cells. In particular, if $\ALPHABET D = \{D_1,\ldots,D_m\}$ is a partition of $Ω$, define
    $$
    σ(\ALPHABET D)
    = \left\{\bigcup_{i \in I}D_i : I \subseteq \{1,\ldots,m\}\right\},
    $$
    where the union corresponding to $I=\emptyset$ is $\emptyset$. Then $σ(\ALPHABET D)$ is the algebra generated by the partition $\ALPHABET D$ and contains $2^m$ events.

    - If $\ALPHABET D = \{Ω\}$, then $σ(\ALPHABET D) = \{\emptyset,Ω\}$.
    - If $\ALPHABET D = \{A,A^c\}$, then $σ(\ALPHABET D) = \{\emptyset,A,A^c,Ω\}$.
    - If $Ω$ is finite and $\ALPHABET D$ is the partition into singleton sets, then $σ(\ALPHABET D) = 2^Ω$.

14. The reverse is also true. If $\ALPHABET F$ is an algebra on a finite space $Ω$, then there is a unique partition $\ALPHABET D = \{D_1, \dots, D_m\}$ such that $\ALPHABET F = σ(\ALPHABET D)$. The elements $D_i$ are called **atoms** of $\ALPHABET F$.

15. Let $\ALPHABET D_1$ and $\ALPHABET D_2$ be two partitions of $Ω$. We say **$\ALPHABET D_1$ is finer than $\ALPHABET D_2$** if $σ(\ALPHABET D_1) \supseteq σ(\ALPHABET D_2)$. For instance, suppose $Ω = \{1, 2, 3, 4\}$ and 
    $$
...
16. If a partition $\ALPHABET D_1$ is finer than $\ALPHABET D_2$, we also say that **$\ALPHABET D_2$ is coarser than $\ALPHABET D_1$**.

## Functions

1. A function $f$ is a rule that assigns each input from a set $\ALPHABET X$ (called the **domain**) to exactly one output in another set $\ALPHABET Y$ (called the **co-domain**). Symbolically, this is written as
   $$
...
3. A function is called **onto** or **surjective** if its range is equal to its co-domain, i.e. for every $y \in \ALPHABET Y$ there exists an $x \in \ALPHABET X$ such that $f(x) = y$.

4. A function is called **one-to-one** or **injective** if distinct inputs give distinct outputs, i.e., $f(x_1) = f(x_2)$ implies $x_1 = x_2$. 
...
Explain if each of the functions is onto, one-to-one, or bijective.
:::
...
3. An **event** is a subset of the sample space. If the outcome of the random experiment belongs to the event $A$, we say that "the event $A$ has occurred". Some examples of events are:
...
4. Note that events are subsets of the sample space but not all subsets of a sample space may be events. The reasons are too complicated to explain, but the high-level explanation is that everything is okay for discrete sample spaces, but weird things can happen in continuous sample spaces. 
...
6. First, to define a function, we need to define its domain and co-domain. Let's denote the domain (i.e., the set of all events to which we can assign a probability) by $\ALPHABET F$. We expect probability to satisfy certain properties, which imposes constraints on the domain:
...
7.  Thus, the domain of $\PR$ **must be an algebra**! However, when we go beyond finite sample spaces, being an algebra is not sufficient. But we first provide some examples of probability for finite sample spaces.
    ...
    A coin toss or die roll modeled with uniform probability is called **fair**: every outcome in $Ω$ has the same probability $1/|Ω|$.
...
10.  We now present an example to illustrate that restricting the domain of $\PR$ to be an algebra is not sufficient.
     ...
     A coin is tossed repeatedly until a head turns up. The sample space is $Ω = \{ω_1, ω_2, \dots\}$ where $ω_n$ denotes the outcome that the first $n-1$ tosses are tails followed by a head. 
     :::
 
     Suppose we are interested in finding the probability of the event that the coin is tossed an even number of times, i.e., $A = \{ω_2, ω_4, \dots\}$. Note that $ω_2, ω_4, \dots \in \ALPHABET F$. However, $A$ is a *countable* set. If we want to assign probability to $A$ in terms of probability of $ω_n$, we require $\ALPHABET F$ to be closed under **countable unions**. This motivates the following definition.
...
12. 
    ...
    So, we restrict to the simplest choice of the domain $\ALPHABET F$ needed for \eqref{eq:uniform} and continuity to hold. [For technical reasons, we need another property known as **completeness**. A probability space can be completed by adding all subsets of null measurable sets. See Sec. 1.6 of the textbook.]{.text-secondary}
    :::
...
14. **$σ$-algebras as information.** A sub-$σ$-algebra $\mathcal G \subseteq \ALPHABET F$ can be interpreted as the information available to an observer. If $A \in \mathcal G$, then this information is sufficient to determine whether or not the event $A$ occurred. Events outside $\mathcal G$ cannot be distinguished using the available information.

    - The smallest $σ$-algebra $\{\emptyset, Ω\}$ represents no information: it distinguishes only the impossible event from the certain event.
    - The $σ$-algebra $\ALPHABET F$ represents all the information modelled by the probability space.
    - If $\mathcal G_1 \subseteq \mathcal G_2$, then $\mathcal G_2$ contains at least as much information as $\mathcal G_1$.

    As information is collected over time, it is natural to consider an increasing sequence
    $$
    \mathcal G_0 \subseteq \mathcal G_1 \subseteq \mathcal G_2 \subseteq \cdots.
    $$
    Such a sequence is called a **filtration**. Later, $σ(X)$ will represent the information revealed by a random variable $X$, while $σ(Y_0,\ldots,Y_n)$ will represent the information contained in observations up to time $n$.

15. A pair $(Ω, \ALPHABET F)$ where $Ω$ is a set and $\ALPHABET F$ is a $σ$-algebra on $Ω$ is called a **measurable space**. 
...
16. 
    ...
    Given a measurable space $(Ω, \ALPHABET F)$, probability 
    $\PR \colon \ALPHABET F \to [0,1]$ is a function that satisfies the following **axioms of probability**

    a) **Non-negativity.** $\PR(A) \ge 0$.
    b) **Normalization.** $\PR(Ω) = 1$.
    c) **Countable additivity.** If $A_1, A_2, \dots \in \ALPHABET F$ is a collection of disjoint events, then,
    $$
...
17. Some immediate implications of the axioms of probability are the following.
    ...
    $$
        ...
        $$
          B = \lim_{n \to ∞} B_n = \bigcap_{n=1}^∞ B_n.
        $$
        Then, 
        $$
...
18. 
    :::{#exm-singleton-probability}
    #### Probability of a singleton

    Let $Ω = [0,1]$, $\ALPHABET F = \mathscr B([0,1])$, and $\PR$ be the uniform distribution on $Ω$. We show that $\PR(\{x\}) = 0$ for every $x \in [0,1]$.

    Define $B_n = [x, x + \tfrac 1n]$. Then $\{B_n\}_{n \ge 1}$ is decreasing and $\lim_{n \to ∞} B_n = \{x\}$ (see @set-limits). Under the uniform distribution, $\PR(B_n) = \tfrac 1n$. By continuity (@lem-probability-properties d), 
    $$
      \PR(\{x\}) = \PR\Bigl(\lim_{n \to ∞} B_n\Bigr) = \lim_{n \to ∞} \PR(B_n) = 0.
    $$
    :::
...
20. Consider $Ω = [0,1]$, $\ALPHABET F = \mathscr B([0,1])$, and $\PR$ to be the uniform probability distribution on $Ω$. Consider the event $A$ that the outcome is a rational number. $A$ is a countable set (because the set of rational numbers is countable). For any $x \in A$, $\{x\} \in \ALPHABET F$, and $\PR(\{x\}) = 0$ (@exm-singleton-probability). Thus, by countable additivity, $\PR(A) = 0$. Hence, $A$ is null.
...
1. Conditional probabilities quantify the uncertainty of an event when it is known that another event has occurred 
   ...
   :::{#exr-conditional-die}
   Suppose we roll a fair six-sided die (a fair die means that all outcomes are equally likely). Consider the events $A$ that the outcome is prime and $B$ that the outcome is a multiple of $3$. Compute $\PR(A | B)$ and $\PR(B | A)$.
   :::
   ...
   sum of the two rolls is less than or equal to $8$ and the event $B$ that the sum of the two rolls is greater than or equal to $6$. Compute $\PR(A|B)$ and $\PR(B|A)$.
   :::
...
2. 
   :::{.callout-tip #conditional-probabilities-are-probabilities}
...
3. The definition of conditional probability gives rise to the chain rule.
   ...
   More generally, let $A_1, A_2, \ldots, A_n$ be events such that
   $\PR(A_1 \cap \cdots \cap A_k) > 0$ for every $k \in \{1,\ldots,n-1\}$. Then
   $$
   ...
   :::{#exr-de-morgan}
   Consider a probability space $(Ω, \ALPHABET F, \PR)$ and events $A, B, C \in \ALPHABET F$ such that $\PR(B^c \cap C^c) > 0$ and $\PR(C^c) > 0$. Prove that
   $$
     \PR(A \cup B \cup C) = 1 - \PR(A^c \mid B^c \cap C^c) \PR(B^c \mid C^c) \PR(C^c).
   $$

   _Hint_: Apply De Morgan's law and the chain rule (@lem-chain-rule).
   :::
...
5. 
   ...
   :::{#exr-sensors}
   An object is present at a location with probability $p$. Conditional on whether the object is present or absent, the detection events of the two sensors are independent. If the object is present, sensor $i$ detects it with probability $d_i$; if the object is absent, neither sensor detects it. If both sensors fail to detect the object, what is the probability that it is present?
   :::

## Independence

1. In general, the knowledge that an event $B$ has occurred changes the probability of event $A$, since $\PR(A)$ is replaced by $\PR(A|B)$. If the knowledge that $B$ has occurred does not change our belief about $A$, i.e., when $\PR(A|B) = \PR(A)$, we say "$A$ and $B$ are **independent**". This leads to the following definition.
   ...
   \PR(B|A) = \PR(B).
   $$
   The above definition is valid only if $\PR(B) > 0$ or $\PR(A) > 0$.
   An alternative but equivalent definition which does not require this condition is
   $$
   ...
   :::{#exr-independence-sigma-algebra}
   Show that $A \independent B$ implies the following:
   ...
   :::{#exr-independence-union}
   Let $A$, $B_1$, and $B_2$ be events such that $A \independent B_1$, $A \independent B_2$, and $A \independent (B_1 \cap B_2)$. Show that $A \independent (B_1 \cup B_2)$.
   :::
...
4. 
   ...
   3. @exr-independence-sigma-algebra implies that independence of $A$ and $B$ implies the independence of $σ(A)$ and $σ(B)$. The reverse implication is trivially true. Thus, independence of events is equivalent to the independence of the smallest $σ$-algebra containing those events.
   :::

5. 
   :::{#def-collective-independence}
 
   A family of events $\{A_1, A_2, \dots, A_n\}$ is called **independent** (sometimes called **mutually independent**) if for all non-empty subsets of indices $\{k_1, \dots, k_m\} \subset \{1,\dots,n\}$, we have
   $$
...
6. 
   ...
   The following example shows that independence is stronger than pairwise independence. Consider an urn with $M$ red balls and $M$ blue balls. Two balls are drawn at random, one at a time, with replacement. Consider the following events:
...
1. So far, we have restricted attention to the outcome of one experiment. It is also possible to construct a probability model that combines the outcomes of two _independent_ experiments. For example, suppose we toss a coin and also roll a die. Let $(Ω_1, \ALPHABET F_1, \PR_1)$ and $(Ω_2, \ALPHABET F_2, \PR_2)$ be the probability spaces associated with the two experiments. What is the probability space $(Ω, \ALPHABET F, \PR)$ of the joint experiment?

2. The sample space of the joint experiment is $Ω = Ω_1 \times Ω_2$. For any $A_1 \in \ALPHABET F_1$ and $A_2 \in \ALPHABET F_2$, the subset $A_1 \times A_2$ of $Ω$ is called a **measurable rectangle**. The collection of measurable rectangles is generally not a $σ$-algebra: for example, a union of two rectangles need not itself be a rectangle. We therefore define the **product $σ$-algebra** by
   $$
   \ALPHABET F_1 \otimes \ALPHABET F_2
   = σ\left(\{A_1 \times A_2 : A_1 \in \ALPHABET F_1,\ A_2 \in \ALPHABET F_2\}\right).
   $$

3. The **product probability measure** $\PR_1 \otimes \PR_2$ is the probability measure on $(Ω_1 \times Ω_2, \ALPHABET F_1 \otimes \ALPHABET F_2)$ characterized by
   $$
   (\PR_1 \otimes \PR_2)(A_1 \times A_2)
   = \PR_1(A_1)\PR_2(A_2),
   \qquad A_1 \in \ALPHABET F_1,\ A_2 \in \ALPHABET F_2.
   $$
   It can be shown that this prescription extends uniquely from measurable rectangles to the entire product $σ$-algebra. The resulting probability space is called the **product** of $(Ω_1, \ALPHABET F_1, \PR_1)$ and $(Ω_2, \ALPHABET F_2, \PR_2)$ and is written as
   $$
   (Ω, \ALPHABET F, \PR)
   = (Ω_1 \times Ω_2, \ALPHABET F_1 \otimes \ALPHABET F_2, \PR_1 \otimes \PR_2).
   $$

   In the familiar finite case where $\ALPHABET F_i = 2^{Ω_i}$, the product $σ$-algebra is $2^{Ω_1 \times Ω_2}$. Thus, the probability of an arbitrary event is obtained by summing the probabilities of the individual outcomes that it contains.

4. Given $A_1 \in \ALPHABET F_1$ and $A_2 \in \ALPHABET F_2$, define the coordinate events
   $$
   ...
   $$
   Thus, events determined by the first coordinate are independent of events determined by the second coordinate. This is precisely how the product probability measure models the independence of the two experiments.

   :::{#exr-coin-tosses}
   A fair coin is tossed $n$ times; the tosses are mutually independent. Find the probability that:

   - a head appears for the first time on the $n$-th toss;
   - the number of heads so far equals the number of tails;
   - exactly two heads have appeared so far;
   - at least two heads have appeared so far.
   :::

5. We will not worry about the measure-theoretic details of the extension from rectangles to the product $σ$-algebra, but will use product probability spaces throughout the course when combining independent random experiments.

## Notes {-}

The material for this section is fairly standard and adapted from various sources. Both Grimmett and Stirzaker and Gubner have an excellent coverage of this material. The idea of categorizing random phenomena as noise, uncertainty, or randomness is borrowed from [Maxim Raginsky's course notes](https://maxim.ece.illinois.edu/teaching/spring16/notes/lec01.pdf). See the book [Against the Gods](https://a.co/d/gVfRocK), which provides a fascinating historical account of the development of probability theory and, why a conceptual leap was needed to recognize that uncertainty could be modeled and measured. 
