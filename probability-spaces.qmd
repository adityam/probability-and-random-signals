...
b) **Countable additivity.** If $A_1, A_2, \dots Ω$ is a collection of disjoint events in $\ALPHABET F$, then,
$$
...
:::{#exr-conditional-dice}
Suppose we roll a fair six-sided dice (a fair dice means that all outcomes are equally likely). Consider the events $A$ that the outcomes is prime and $B$ that the outcome is a multiple of $3$. Compute $\PR(A | B)$ and $\PR(B | A)$.
:::
...
Suppose we roll two fair six-sided dice. Consider the event $A$ that the minimum of the two rolls is greater than or equal to $6$ and the event $B$ that the maximum of the two rolls is less than or equal to $8$. Compute $\PR(A|B)$ and $\PR(B|A)$.
:::
...
- For disjoint events $A_1, A_2 \in \ALPHABET F$, 
  $\PR(A_1 \cup A_2 \mid B) = \PR(A_1 \mid B) + \PR(A_2 \mid B)$.
:::
...
:::{#exm-independence-dice}
The events $A$ and $B$ defined in @exr-conditional-dice are independent.
:::
