...
14. Let $\ALPHABET D_1$ and $\ALPHABET D_2$ be two partitions of $Ω$. We say **$\ALPHABET D_1$ is finer than $\ALPHABET D_2$** if $σ(\ALPHABET D_1) \supseteq σ(\ALPHABET D_2)$. For instance, suppose $Ω = \{1, 2, 3, 4\}$ and 
    $$
...
17. 
    ...
    Consider $A_n = \bigl[0, a - \frac 1n \bigr)$ and define $A = \lim_{n \to ∞} A_n$. Since $\{A_n\}_{n \ge 1}$ is an increasing sequence, by @lem-probability-properties (d), we have $A = \bigcup_{n=1}^∞ A_n = [0, a)$. Therefore, $\PR(A) = \lim_{n \to ∞} \PR(A_n)$.

    The same argument holds when $A_n = \bigl[0, a - \frac 1n \bigr]$. We have $\PR(A) = \lim_{n \to ∞} \PR(A_n)$.

    Consider $B_n = \bigl[0, a + \frac 1n \bigr)$ and define $B = \lim_{n \to ∞} B_n$. Since $\{B_n\}_{n \ge 1}$ is a decreasing sequence, by @lem-probability-properties (d), we have $B = \bigcap_{n=1}^∞ B_n = [0,a]$. Therefore, $\PR(B) = \lim_{n \to ∞} \PR(B_n)$.

    The same argument holds when $B_n = \bigl[0, a + \frac 1n \bigr]$. We have $\PR(B) = \lim_{n \to ∞} \PR(B_n)$.
...
19. Consider $Ω = [0,1]$, $\ALPHABET F = \mathscr B([0,1])$, and $\PR$ to be the uniform probability distribution on $Ω$. Consider the event $A$ that the outcome is a rational number. $A$ is a countable set (because the set of rational numbers is countable). For any $x \in A$, $\{x\} \in \ALPHABET F$, and $\PR(\{x\}) = 0$ (see @exr-singleton-probability). Thus, by countable additivity, $\PR(A) = 0$. Hence, $A$ is null.
...
:::{#exr-singleton-probability}
Let $Ω = [0,1]$, $\ALPHABET F = \mathscr B([0,1])$, and $\PR$ be the uniform distribution on $Ω$. For any $x \in [0,1]$, show that $\PR(\{x\}) = 0$.

_Hint_: Write $\{x\}$ as the limit of a decreasing sequence of intervals.
:::
...
5. 
   ...
   :::{#exr-sensors}
   An object is present at a location with probability $p$. Two sensors operate independently. If the object is present, sensor $i$ detects it with probability $d_i$; if absent, neither sensor detects it. If both sensors fail to detect the object, what is the probability that it is present?
   :::
...
2. The sample space should obviously be $Ω = Ω_1 \times Ω_2$. When $\ALPHABET F_1$ and $\ALPHABET F_2$ are finite, then we can simply define $\ALPHABET F = \ALPHABET F_1 \times \ALPHABET F_2$ and for any $A = A_1 \times A_2 \in \ALPHABET F$, $\PR(A)$ to be $\PR_1(A_1) \PR_2(A_2)$. Note that $\PR(Ω) = \PR_1(Ω_1)\PR_2(Ω_2) = 1$, thus $(Ω, \ALPHABET F, \PR)$ is a valid probability space. This space is called **direct product** of probability spaces $(Ω_1, \ALPHABET F_1, \PR_1)$ and $(Ω_2, \ALPHABET F_2, \PR_2)$. 
...
5. It is a bit more complicated when $\ALPHABET F_1$ and $\ALPHABET F_2$ are not finite. The difficulty is that $\ALPHABET F_1 \times \ALPHABET F_2$ is not a $σ$-algebra. So, take $\ALPHABET F$ to be $σ(\ALPHABET F_1 \times \ALPHABET F_2)$ (which is the smallest $σ$-algebra containing $\ALPHABET F_1 \times \ALPHABET F_2$) and define $\PR$ to be the extension of $\PR_1 \times \PR_2$ from $\ALPHABET F_1 \times \ALPHABET F_2$ to $σ(\ALPHABET F_1 \times \ALPHABET F_2)$ (one can show that such an extension exists). Such product space is often written as
$$
