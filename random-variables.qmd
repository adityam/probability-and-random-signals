...
4. The **cumulative distribution function (CDF)** of a (real-valued) random variable $X$ is the function $F \colon \reals \to [0,1]$ given by
   $$
   F(x) \coloneqq \PR(X \le x) = \PR(\{ ω \in Ω : X(ω) \le x \}).
   $$
...
5. When we need to emphasize the dependence of the CDF on the random variable, we use the notation $F_X$, etc. 

6. **Some examples of CDF of random variables**
...
2. A random variable $X$ is called **continuous** if there exists an integrable function $f \colon \reals \to [0, ∞)$ called the **probability denisity function** such that the CDF can be written as
   ...
   ![CDF of a continuous random variable](figures/svg/cdf4.svg)
...
3. A random variable is called **mixed** if it is neither discrete nor continuous. [For a mixed random variable, the CDF has has jumps at a finite or countable infinite number of points and it is continuous over one or many intervals.]{.text-primary}
   ...
   ![CDF of a mixed random variable](figures/svg/cdf5.svg)
...
A discrete random variable creates a partition of the sample space. In particular, suppose $X$ is a random variable and $\ALPHABET X = \{x_1, x_2, \dots\}$ is the range of $X$. Define
...
The power-set of $\{A_1, A_2, \dots\}$ is called the **$σ$-algebra generated by $X$** and denoted by $σ(X)$. This $σ$-algebra captures the crux of measurability. As an illustration, let's reconsider @exm-coin-toss. In this case, the range of $X$ is $\ALPHABET X = \{0, 1, 2\}$. The partition corresponding to $σ(X)$ is shown in @fig-sigma-X.

![Illustration of $σ(X)$ for @exm-coin-toss](figures/svg/probability5.svg){#fig-sigma-X}
:::
...
- **Properties of continuous random variables**
  ...
  $$
  By construction, $f(x) \ge 0$ and $\int_{-∞}^{∞} f(x)\, dx = 1$.

  Then, for any event $A \in \mathscr{B}(\reals)$, 
  $$\PR(X \in A) = \int_{x \in A} f(x)\,dx.$$ 
:::
