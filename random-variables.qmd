...
5. For the above probability calculations to make sense, we require that $X^{-1}(B) \in \ALPHABET F$ whenever $B \in \ALPHABET G$. This property is called **measurability** and a function with this property is called **$\ALPHABET F/\ALPHABET G$-measurable**. 
...
5. **Some examples of CDF of random variables**
   ...
   A Bernoulli random variable takes two possible values: value $0$ with probability $1-p$ and value $1$ with probability $p$. Its CDF is given by
   $$
   F_X(x) = \begin{cases}
   0, & x < 0 \\
   1 - p, & 0 \le x < 1 \\
...
6. 
   ...
   3. Define the increasing sequence of events $A_n = \{ X \le n\}$, $n \in \naturalnumbers$. By continuity of probability, we have
      ...
      \end{align*}
         ...
         The reverse argument is similar where we consider the decreasing sequence of events $B_n = \{ X \le -n \}$, $n \in \naturalnumbers$. Then, by continuity of probability, we have
         \begin{align*}
   ...
   5.  Consider the decreasing sequence of sets:
       ...
       \begin{align*}
         &\quad & \PR\biggl( \bigcap_{n=1}^{∞} A_n \biggr) 
         & = \lim_{n \to ∞} \PR(A_n) \\
...
2. A random variable $X$ is called **continuous** if there exists an integrable function $f_X \colon \reals \to [0, ∞)$ called the **probability density function** such that the CDF can be written as
    $$
...
3. A random variable is called **mixed** if it is neither discrete nor continuous. [For a mixed random variable, the CDF has jumps at a finite or countable infinite number of points and it is continuous over one or many intervals.]{.text-primary}
...
A Binomial random variable is the sum of independent and identically distributed Bernoulli random variables (we will prove this fact later). For example, if a biased coin (with $\PR(H) = p$) is tossed $n$ times, then the number of heads is a _binomial random variable with parameters $n$ and $p$_, which is denoted by $\text{Binomial}(n,p)$. For such a random variable,
$$
...
A geometric random variable is the number of trials in i.i.d.\ Bernoulli random variables. For example, if a biased coin (with $\PR(H) = p$) is tossed repeatedly, the number of tosses needed for the first head is a geometric random variable with parameter $p \in (0,1)$, which is denoted by $\text{Geo}(p)$. For such a random variable,
$$
P_X(k) = (1-p)^{k-1} p, \quad k \in \integers_{> 0}.
$$
:::

Geometric random variables have the property of being **memoryless**: the distribution of waiting time does not depend on how much time has already elapsed. 


:::{#exm-Poisson}
#### Poisson random variable

Poisson random variables model many different phenomenon ranging from photoelectric effect in photonics to inter-packet arrival times in computer networks. A random variable is said to be a _Poisson random variable with parameter $λ > 0$_, which is denoted by $\text{Poisson}(λ)$, if
$$
...
Poisson random variables are **closed under addition**: the sum of independent Poisson random variables is Poisson.
...
A Gaussian distribution is also called a **Normal distribution**. When $μ = 0$ and $σ^2 = 1$, the distribution is called **standard Normal**. We write $ϕ$ for its density and $Φ$ for its CDF:
\begin{align*}
ϕ(z)
&=\frac{1}{\sqrt{2π}}\,e^{-z^2/2},
\qquad z \in \reals,
\\
Φ(z)
&=\int_{-∞}^{z} ϕ(t)\,dt.
\end{align*}
Thus, if $X \sim \mathcal N(μ,σ^2)$, then
$$
f_X(x)=\frac{1}{σ}\,ϕ\Bigl(\frac{x-μ}{σ}\Bigr),
\qquad
F_X(x)=Φ\Bigl(\frac{x-μ}{σ}\Bigr).
$$
:::

The Gaussian distribution is perhaps the most important continuous distribution because of its role in the **Central Limit Theorem**. Gaussian random variables are **closed under addition**: the sum of independent Gaussian random variables is Gaussian.
...
- The $σ$-algebra generated by this partition is called the **$σ$-algebra generated by $X$** and denoted by $σ(X)$. This $σ$-algebra captures the crux of measurability. 
...
![Illustration of the partition created by $Y = g(X)$ for @exm-coin-toss](figures/svg/probability6.svg){#fig-sigma-Y}
...
```{ojs}
//| fig-cap: The function $g(x)$ for @exm-function-uniform-1
//| label: fig-function-uniform-1
...
3. Now consider a $y \in (0,1)$. We have
   ...
   Thus,
   $$
    F_Y(y) = F_X(y) + 1 -  F_X(2-y) = \frac {y}{2} + 1 - \frac{2-y}{2} = y. 
   $$
   Thus, 
   $$
...
\end{cases}
$$
Define $Y = g(X)$. Find $F_Y(y)$. Does $Y$ have a PDF?
...
```{ojs}
//| fig-cap: The function $g(x)$ for @exm-function-uniform-2
//| label: fig-function-uniform-2
...
The range of $g$ is $[0,1]$. Therefore, $F_Y(y)=0$ for $y<0$ and $F_Y(y)=1$ for $y\geq 1$.

Now consider $0 \leq y<1$. Except for endpoints, which have probability zero, the event $\{Y\leq y\}$ occurs when either $X\leq y$ or $X\geq 4-y$. Hence,
\begin{align*}
F_Y(y)
&= \PR(X\leq y)+\PR(X\geq 4-y) \\
&= \frac{y}{4}+\frac{y}{4}
= \frac{y}{2}.
\end{align*}
Thus,
...
The CDF has a jump of size $1/2$ at $y=1$, so
$$
\PR(Y=1)=F_Y(1)-F_Y(1^-)=\frac12.
$$
This atom arises because $g(x)=1$ for $x\in(1,3)$, an interval having probability $1/2$. On $(0,1)$, the continuous component has density
$$
f_Y(y)=\frac12, \qquad 0<y<1.
$$
The density integrates to $1/2$; the remaining probability mass $1/2$ is concentrated at $y=1$. Thus, $Y$ has a mixed distribution and cannot be described by an ordinary PDF alone.
:::
...
= \PR(X \le σ z + μ)
\\
&= \int_{-∞}^{σ z + μ} \frac{1}{σ}\,ϕ\Bigl(\frac{x-μ}{σ}\Bigr)\, dx
\\
&= \int_{-∞}^{z} ϕ(y)\, dy
= Φ(z),
\end{align*}
...
Thus $f_Z(z)=ϕ(z)$, so $Z \sim \mathcal{N}(0,1)$.
:::

:::{#exm-generating-random-variables}
#### Generating non-uniform random variables by inverting the CDF

Suppose $F \colon \reals \to [0,1]$ is a function that satisfies the following properties:
...
1. Suppose $g$ is a continuously differentiable and one-to-one function (from $\text{Range}(X)$ to $\text{Range}(Y)$). Thus, $g$ must be either strictly increasing or strictly decreasing, and in both cases the inverse $g^{-1}$ is well defined. 
...
4. The change of variables formula can be used to verify some common ways of generating non-uniform random variables from uniform random variables. For example: 

    - Suppose $U \sim \text{Uniform}(0,1)$ and $λ > 0$. Then, $X = - \ln(1 - U)/λ$ has $\text{exp}(λ)$ distribution.

   Note that the change of variable formula only works for continuous function. For non-continuous functions, we need to use first principles (or the idea in @exm-generating-random-variables). For example:

    - Suppose $U \sim \text{Uniform}(0,1)$ and $p \in (0,1)$. Then $X = \IND_{\{U \le p\}}$ has $\text{Bernoulli}(p)$ distribution.

    See [Devroye (1986)](https://doi.org/10.1007/978-1-4613-8643-8) for a general discussion on generating non-uniform random variables,
...
| Geometric       | $p$          | $\dfrac 1p$ | $\dfrac{1-p}{p^2}$ |
| Poisson         | $λ$          | $λ$  | $λ$      |
...
:::{#exm-streaming-variance}
Let $X$ be a real-valued random variable where $μ = \EXP[X]$. Let $t$ be any real number. Then show that
$$
...
$$
 ...
 _Remark:_ The above relationship suggests a **single-pass** algorithm to compute $\VAR(X)$ from a set of samples: choose any $t$ and compute $μ = \EXP[X]$ and $\EXP[(X-t)^2]$ in a single pass. Then, $\VAR(X)$ can be computed as $\VAR(X) = \EXP[(X-t)^2] - (t-μ)^2$.
:::
...
\end{align*}
where $(a)$ uses the linearity property of expectations and $(b)$ uses
the fact that $\EXP[X-μ] = μ -μ = 0$. 
:::
