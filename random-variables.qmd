...
1. A random variable $X$ is said to be **discrete** if it takes values in a countable subset $\{x_1, x_2, \dots\}$ of $\reals$. A discrete random variable has a **probability mass function (PMF)** $f \colon \reals \to [0,1]$ which satisfies the following properties:
    ...
    - $f(x) = \PR(X = x) = F(x) - F(x^{-})$.
    - $F(x) = \sum_{x_n : x_n \le x} f(x_n).$
    - An implication of the above is $\sum_{n=1}^{∞} f(x_n) = 1$. 
...
A Binomial random variable is the sum of intendant and identically distributed random variables (we will prove this fact later). For example, if a biased coin (with $\PR(H) = p$) is tossed $n$ times, then the number of heads is a _binomial random variable with parameters $n$ and $p$_, which is denoted by $\text{Binomial}(n,p)$. For such a random variable,
$$
f(k) = \binom n k p^k (1-p)^{n-k}, \quad 0 \le k \le n.
$$
...
$$
f(k) = \frac{λ^k}{k!} e^{-λ}, \quad k \in \integers_{\ge 0}.
$$
:::

:::{#exm-Geometric}
#### Geometeric random variable

A random variable is said to _Geometric random variable with parameter $p \in (0,1)$_, which is denoted by $\text{Geo}(p)$, if
$$
f(k) = (1-p)^{k-1} p, \quad k \in \integers_{> 0}.
$$
:::
...
$$f(k) = \frac 1{\ABS{\ALPHABET S}}, \quad k \in \ALPHABET S.$$
:::
...
When $X$ is discrete and takes values $\{x_1, x_2, \dots, x_n\}$, we expect that the number of times we obtain a value $x_i$ is approximately $Nf(x_i)$ when $N$ is large. Thus,
$$ m \approx \frac 1N \sum_{i=1}^n x_i \, N f(x_i)  = \sum_{i=1}^n x_i f(x_i). $$
...
$$
f(n) = f(-n) = \frac {1}{2C n^2}, \quad n \in \naturalnumbers 
$$
