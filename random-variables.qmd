...
The above equation shows that for any arbitrary (Borel) subsets $B_1$ and $B_2$ of $\reals$, $\PR(X \in B_1, Y \in B_2) = \PR(X \in B_1) \PR(Y \in B_2)$. Hence, $\{X \in B_1\} \independent \{Y \in B_2 \}$. Since $B_1$ and $B_2$ were arbirary, we have $X \independent Y$. 
:::
...
- The **correlation matrix** of a random vector $X \in \reals^n$ is defined as $\EXP[X X^\TRANS]$, where $X^\TRANS$ denotes the transpose of $X$. 

- The **covariance matrix** of a ranom vector $X \in \reals^n$ is defined as $\EXP[ (X - μ_X) (X - μ_X)^\TRANS]$.
...
- Both the coorelation and covariance matrices are positive semidefinite. Thus, their eigenvalues are real and non-negative. 
...
- Since $Y$ is a random variable, it is poosible to compute its CDF and PMF/PDF as appropriate. We illustrate this concept via some examples.


:::{#exm-function-uniform}
...
:::{#exr-function-sum-independent}
Repeat @exr-function-sum when $X_1$ and $X_2$ are independent. 
:::
...
For continuous random variables, it is possible to obtain a general **change of variable** formula to obtain the PDF of functions of random variable in terms of their joint PDF. See Sec 4.7 of the textbook for details. In my view, it is usually easier to simply reason about such change of variables from first principles. 
:::
