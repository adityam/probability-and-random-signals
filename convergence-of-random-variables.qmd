...
- In @exm-converge-4, we can directly apply @lem-as to argue that $Y_n \xrightarrow{a.s.} 0$.
...
2. There is a partial converse. For any constant $c$
   $$
     [X_n \xrightarrow{D} c] 
     \implies
     [X_n \xrightarrow{p} c].
...
4. Let $f \colon \reals \to \reals$ be a continuous function. Then,
   
    - $X_n \xrightarrow{a.s.} X^*$ implies $f(X_n) \xrightarrow{a.s.} f(X^*)$.
    - $X_n \xrightarrow{p} X^*$ implies $f(X_n) \xrightarrow{p} f(X^*)$.
    - $X_n \xrightarrow{D} X^*$ implies $f(X_n) \xrightarrow{D} f(X^*)$.

5. If $X_n \xrightarrow{p} X$ and $Y_n \xrightarrow{p} Y$, then $X_n + Y_n \to X + Y$ and  $X_n Y_n \xrightarrow X Y$.
