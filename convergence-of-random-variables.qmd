...
:::{#exm-convergence-2}
Consider a probability space as in @exm-convergence-1. Define $X_n(ω) = \IND_{A_n}(ω)$, where $A_n = \{ ω : n ω - \lfloor n ω \rfloor \in [0, \frac 1n] \}$.
...
6. Convergence in mean-square and almost surely are not comparable. @exm-convergence-3, part (c) gives an example that converges almost surely but not in mean square. @exm-convergence-5, part (a) gives an example that converges in mean square but not almost surely.
...
6. **Slutsky's theorem:** Suppose $X_n \xrightarrow{D} X$ and $Y_n \xrightarrow{D} c$, where $c$ is a constant. Then, $(X_n, Y_n) \xrightarrow{D} (X, c)$. Therefore, for any continuous function $g(X_n, Y_n) \xrightarrow{D} g(X, c)$. In particular, $X_n + Y_n \xrightarrow{D} X + c$ and $X_n Y_n \xrightarrow{D} cX$. 
...
$$\PR(A_n) = \PR\bigl( \ABS{X_n} > ε \bigr)
\le \dfrac{\EXP[\ABS{X_n}]}{ε}. $$
Therefore,
$$ \sum_{n=1}^{∞} \PR(A_n) 
...
$$
where $M_4$ is the fourth moment. Now, from the fourth moment version of Markov inequality, we have
