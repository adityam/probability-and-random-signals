...
1. We construct a continuous-time Markov chain as follows. Whenever the chain enters state $i$, it remains there for an exponentially distributed **holding time** with rate $ν_i$. At the end of the holding time, it jumps to state $j$ with probability $P_{ij}$, where
   $$ P_{ii} = 0, \quad \sum_{j \ne i} P_{ij} = 1. $$
...
## Local transition rates and the generator
...
3. We collect the transition rates in the **generator matrix** $Q$, where
   $$ Q_{ij} = q_{ij} \quad (j \ne i), \qquad Q_{ii} = -\sum_{j \ne i} q_{ij} = -ν_i. $$
   The off-diagonal entries of $Q$ are nonnegative and every row sums to zero:
   $$ Q\mathbf 1 = \mathbf 0. $$
   If $P(h)$ denotes the matrix of transition probabilities over an interval of length $h$, then
   $$ P(h) = I + hQ + o(h). $$

4. :::{#exm-two-state-ctmc}
   ...
   $$ Q = \MATRIX{-α & α \\ β & -β}. $$
   For example, this can model a component that fails at rate $α$ and is repaired at rate $β$.
   :::

## State transition diagrams

1. As in discrete time, we visualize continuous-time Markov chains by **state transition diagrams**. The drawing convention is the same, with one difference: the edge labels are the off-diagonal rates $q_{ij}$ from $Q$, and we **do not draw self-loops**. In discrete time, a self-loop at $i$ records $P_{ii}$. In continuous time, remaining in $i$ for a short interval is already encoded by the holding rate $ν_i = -Q_{ii}$, so a self-loop is redundant.

   Given a diagram, we recover the generator by reading $Q_{ij} = q_{ij}$ from the directed edge $i \to j$ (for $j \ne i$) and setting
   $$ Q_{ii} = -\sum_{j \ne i} q_{ij}. $$
   The holding rates and embedded jump probabilities then follow from
   $$ ν_i = -Q_{ii}, \qquad P_{ij} = \frac{q_{ij}}{ν_i} \quad (j \ne i), \qquad P_{ii} = 0, $$
   whenever $ν_i > 0$.

2. :::{#exm-ctmc-diagram-two-state}
   #### From $Q$ to a diagram: two states

   The two-state chain of @exm-two-state-ctmc has generator
   $$ Q = \MATRIX{-α & α \\ β & -β}. $$
   Its diagram has an edge $0 \to 1$ labeled $α$ and an edge $1 \to 0$ labeled $β$. There are no self-loops.

   ![Two-state continuous-time Markov chain](figures/svg/ctmc-examples1.svg)
   :::

3. :::{#exm-ctmc-diagram-three-state}
   #### From $Q$ to a diagram: three states

   Consider
   $$ Q = \MATRIX{-2 & 1 & 1 \\ 2 & -3 & 1 \\ 0 & 2 & -2}. $$
   The off-diagonal entries become edge labels: $0 \to 1$ and $0 \to 2$ at rate $1$, $1 \to 0$ at rate $2$, $1 \to 2$ at rate $1$, and $2 \to 1$ at rate $2$. There is no edge $2 \to 0$, since $Q_{20} = 0$. There are no self-loops; the diagonal entries $-2$, $-3$, and $-2$ are minus the total outgoing rate from each state.

   ![Three-state continuous-time Markov chain](figures/svg/ctmc-examples2.svg)
   :::

4. :::{#exm-finite-buffer-queue}
...
1. For a nonabsorbing state $i$, there is a directed edge $i \to j$ in the continuous-time chain whenever $q_{ij} > 0$, equivalently whenever $P_{ij} > 0$ in the embedded chain. Thus the continuous-time chain and its embedded chain have the same accessibility relations, communicating classes, closed classes, and irreducibility. A state with $ν_i = 0$ is an absorbing singleton class; its row in $P$ remains irrelevant. We can therefore read the class structure from the embedded chain, using the results for discrete-time Markov chains. Periodicity, however, is not a condition that we carry over to continuous time.
...
1. Define the transition probabilities
   ...
   $$ P(0) = I. $$
   The notation $P(t)$ should not be confused with the matrix $P$ of the embedded jump chain.

2. Conditioning on the state after time $s$ gives the **Chapman--Kolmogorov equations**
   $$ P_{ij}(s + t) = \sum_{k \in \ALPHABET X} P_{ik}(s) P_{kj}(t), $$
   or, in matrix notation,
   $$ P(s + t) = P(s) P(t). $$

3. Combining the Chapman--Kolmogorov equations with
   $$ P(h) = I + hQ + o(h) $$
...
4. Let $μ(t)$ be the row vector of state occupancy probabilities,
   $$ μ_i(t) = \PR(X(t) = i). $$
   Then
   $$ μ(t) = μ(0) P(t), $$
...
5. For the two-state chain in @exm-two-state-ctmc,
   $$
     P(t) = \frac{1}{α + β}
     \MATRIX{
