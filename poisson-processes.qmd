...
A Bernoulli process is a sequence $\{Z_n\}_{n \ge 1}$ of i.i.d. binary random variables. Let $p = \PR(Z_n = 1)$ and $q = 1 - p = \PR(Z_n = 0)$. 

An example would be i.i.d. coin tosses where we can think of $\{Z_n = 1\}$ as the event that the $h$-th outcome is a head and $\{Z_n = 0\}$ to be the event that the $n$-th outcome is a tail.

Given a Bernoulli process, define $U_1$ to be the first time $n$ such that $Z_n = 1$; $U_2$ to be the first time $n$ after $U_1$ such that $Z_n = 1$, etc. The sequence $\{U_n\}_{n \ge 1}$ is called _inter-arrival times_. It is easy to see that $$ P_{U_1}(k) = (1 - p)^{k-1} p. $$ Due to the independence of the outcomes, we can also argue that $$ P_{U_2}(k) = (1-p)^{k-1} p, $$ and so on for all $U_n$. Thus, $U_n \sim \text{Geo}(p)$. 
...
$$
  N_n = \sum_{i=1}^n U_i
$$
to be the time of the $n$-th arrival. Note that the process $\{N_n\}_{n \ge 1}$ is a sequence of increasing random variables. Such a sequence is called an _arrival process_. Given an arrival process, we can construct inter-arrival times as $U_n = N_n - N_{n-1}$. Then, the Bernoulli process is an arrival process with geometric (or, equivalently, memoryless) interarrival times.

Note that the arrival times $\{N_n\}_{n \ge 1}$ take integer values. If we allow the arrival times to take continuous values, we get a Poisson process.

# Poisson Processes

A Poisson process is an arrival process $\{N_n\}_{n \ge 1}$ (i.e., an
increasing sequence of random variables) such that the inter-arrival times
$\{U_n\}_{n \ge 1}$, where $U_n = N_n - N_{n-1}$, are independent and
memoryless. Since a non-negative continuous time random variable which is
memoryless must have an exponential distribution (see @exr-poisson below), we
