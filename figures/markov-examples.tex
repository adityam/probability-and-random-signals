...
% State augmentation for a two-slot burst-erasure channel
% Left to right: first bad slot, second bad slot, good state
\startTEXpage[offset=1dk]
\scale[scale=1500]{\starttikzpicture[line width=1bp]
   \matrix[matrix of nodes, nodes={state}, column sep=1.8cm, row sep=1.5cm] (m) {
    \m{(0,0)} & \m{(0,1)} & \m{1} \\
  };
  \draw[every loop]
       (m-1-3) edge[loop right] node {$1-p$} (m-1-3)
       (m-1-1) edge[bend left, auto=left] node {$1$} (m-1-2)
       (m-1-2) edge[bend left, auto=left] node {$1$} (m-1-3)
       (m-1-3) edge[bend left=35, auto=left] node {$p$} (m-1-1)
       ;
