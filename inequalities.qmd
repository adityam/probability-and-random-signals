...
:::{#exr-union-bound-infinite-events}
...
@exr-Markov shows that the Markov inequality is not tight. Moreover, it gives a vaccous bound for $a < μ$. Thus, it only makes sense to apply the Markov inequality of $a \ge μ$.  
...
Chernoff bound is much stronger than Markov and Chebyshev inequalities. To see that, let's revisit the bound of @exr-Binomial-comparison. For a Binomial random variable, 
$$ 
M_X(s) = (1 - p + p e^s)^n.
$$
Then,
...
$$
Even before we go ahead and compute the value in maximum of the term in square brackets, we observe that the result here is qualitatively different from that in @exr-Binomial-comparison. As for Chebyshev inequality, we get that the probability is going to zero, but Chernoff bound shows that the probability is going to zero _exponentially fast_. A bit of algebra shows that the exact bound is
$$
...
We can revisit @exr-Binomial-comparison using Hoeffding inequality. Recall that a Binomial random variable is the sum of i.i.d. Bernoulli random variables. Therefore, we have
$$
